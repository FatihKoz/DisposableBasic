...
class DB_AirportServices
{
  ...
  public function GetPireps($location, $count = null)
  {
    ...
    $pireps = Pirep::with($eager_load)
      ->where(function($query) use($location) { $query->where('dpt_airport_id', $location)->orWhere('arr_airport_id', $location); })
      ->where($where)
      ->whereIn('airline_id', $airlines_array)
      ->whereIn('user_id', $users_array)
      ->orderBy('submitted_at', 'desc')
      ->when(is_numeric($count), function($query) use($count) { return $query->take($count); })
      ->get();
