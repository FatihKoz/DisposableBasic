...
class DB_StatServices
{
  ...
  public function LeaderBoard($source, $count, $type, $period, $hub)
  {
    ...
    if ($source === 'airline') { // Airline
      ...
      $eager_load = 'user';
      $user_where = [];
      if (isset($hub)) { $user_where['home_airport_id'] = $hub; }
    ...
    if ($type === 'distance') { $select_Raw = 'sum(distance)'; }
    elseif ($type === 'time') { $select_Raw = 'sum(flight_time)'; }
    elseif ($type === 'lrate') { $select_Raw = 'avg(landing_rate)'; }
    elseif ($type === 'lrate_low') { $select_Raw = 'max(landing_rate)'; }
    elseif ($type === 'lrate_high') { $select_Raw = 'min(landing_rate)'; }
    elseif ($type === 'score') { $select_Raw = 'avg(score)'; }
    else { $select_Raw = 'count(id)'; }

    // Main Query
    $results = Pirep::with($eager_load)->selectRaw($base.', '.$select_Raw.' as totals')
      ->where($where)
      ->when(isset($is_period), function($query) use ($s_date, $e_date) { $query->whereBetween('created_at', [$s_date, $e_date]); })
      ->when(is_array($whereIn_array), function($query) use ($base, $whereIn_array) { $query->whereIn($base, $whereIn_array); })
      ->when(($type != 'lrate_high'), function($query) { $query->orderBy('totals', 'desc'); }, function($query) { $query->orderBy('totals', 'asc'); })
      ->groupBy($base)->take($count)->get();

    // Route
    if ($source === 'airline') { $route = 'DBasic.airline'; }
    elseif ($source === 'dep' | $source === 'arr') { $route = 'frontend.airports.show'; }
    else { $route = 'frontend.profile.show'; }

    // Leader Board
    $leader_board = [];
    foreach ($results as $item) {

      if ($type === 'time') { $item->totals = DB_ConvertMinutes($item->totals, '%02dh %02dm'); }
      elseif ($type === 'lrate' || $type === 'lrate_low' || $type === 'lrate_high') { $item->totals = number_format($item->totals).' ft/min'; }
      elseif ($type === 'distance') { $item->totals = DB_ConvertDistance($item->totals); }
      else { $item->totals = number_format($item->totals); }
  ...
  public function PirepStats($airline_id = null, $aircraft_id = null)
  {
    ...
    */
    ...
    if ($unit_distance === 'km') {
      ...
      if ($level > 10) { $average_dist = $average_dist * 1.852; }
    } elseif ($unit_distance === 'mi') {
      $total_dist = $total_dist * 1.15078;
      if ($level > 10) { $average_dist = $average_dist * 1.15078; }
    }

    if ($unit_fuel === 'kg') {
      $total_fuel = $total_fuel / 2.20462262185;
      if ($level > 10) { $average_fuel = $average_fuel / 2.20462262185; }
    }

    $stats[__('DBasic::stats.ttime')] = DB_ConvertMinutes($total_time, '%02dh %02dm');
    if ($level > 10) { $stats[__('DBasic::stats.atime')] = DB_ConvertMinutes($average_time, '%02dh %02dm'); }

    $stats[__('DBasic::stats.tfuel')] = number_format($total_fuel).' '.$unit_fuel;
    if ($level > 10) { $stats[__('DBasic::stats.afuel')] = number_format($average_fuel).' '.$unit_fuel; }
    ...
    if ($level > 10) { $stats[__('DBasic::stats.adist')] = number_format($average_dist).' '.$unit_distance; }
