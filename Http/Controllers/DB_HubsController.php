...
class DB_HubsController extends Controller
{
  ...
  public function show($id)
  {
    ...
    if ($hub) {
      ...
      $aircraft_hub = Aircraft::with('subfleet.airline')->whereIn('subfleet_id', $hub_subfleets)->orderby('icao')->orderby('registration')->get();
      $aircraft_off = Aircraft::with('subfleet.airline')->whereNotIn('subfleet_id', $hub_subfleets)->where('airport_id', $hub->id)->orderby('icao')->orderby('registration')->get();
      ...
      $users_hub = User::with('airline')->where($hub_where)->orderby('id')->get();
      $users_off = User::with('airline')->where($off_where)->orderby('id')->get();
      $is_visible['pilots'] = ($users_hub->count() > 0 || $users_off->count() > 0) ? true : false;

      // Reports
      $p_where = array('state' => 2, 'status' => 'ONB');
      $pireps = Pirep::with('user', 'aircraft', 'airline')->where('dpt_airport_id', $hub->id)->where($p_where)->orWhere('arr_airport_id', $hub->id)->where($p_where)->orderby('submitted_at', 'desc')->paginate(100);
      $is_visible['reports'] = ($pireps->count() > 0) ? true : false;

      // Downloads
      $is_visible['downloads'] = ($hub->files->count() > 0 && Auth::check()) ? true : false;
