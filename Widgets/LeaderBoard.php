...
class LeaderBoard extends Widget
{
  ...
  public function run()
  {
    ...
    if ($period === 'currentm') { $period_text = $now->startOfMonth()->format('F'); }
    elseif ($period === 'lastm') { $period_text = $now->subMonthNoOverflow()->startOfMonth()->format('F'); }
    elseif ($period === 'prevm') { $period_text = $now->subMonthsNoOverflow(2)->startOfMonth()->format('F'); }

    elseif ($period === 'currenty') { $period_text = $now->startOfYear()->format('Y');}
    elseif ($period === 'lasty') { $period_text = $now->subYearNoOverflow()->startOfYear()->format('Y'); }
    elseif ($period === 'prevy') { $period_text = $now->subYearsNoOverflow(2)->startOfYear()->format('Y'); }

    if (isset($period_text)) { $title = $title.' | '.$period_text; }

    // Type text (visible at Card Footer)
    if ($type === 'distance') { $type_text = __('DBasic::common.distance'); }
    elseif ($type === 'time') { $type_text = __('DBasic::common.btime'); }
    elseif ($type === 'lrate') { $type_text = __('DBasic::common.lrate'); }
    elseif ($type === 'lrate_low') { $type_text = __('DBasic::widgets.lrate_low'); }
    elseif ($type === 'lrate_high') { $type_text = __('DBasic::widgets.lrate_hgh'); }
    elseif ($type === 'score') { $type_text = __('DBasic::common.score'); }
    else { $type_text = __('DBasic::common.flights'); }
